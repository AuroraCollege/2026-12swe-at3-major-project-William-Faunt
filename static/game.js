...
async function loadState() {
  ...
  if (mode === "pvc" && gameState.turn === gameState.ai_side) {
    await aiMove();
  }
}
...
document.addEventListener("keydown", (e) => {
  ...
  const elapsed = performance.now() - qteStartTime;
  if (elapsed > qteDuration) return; // already expired

  if (e.key === expectedKey) {
    registerHit(elapsed);
    nextPromptOrFinish();
  } else {
    // ignore wrong keys (or treat as miss if you want)
  }
});
